...
function statusBadge(status: TradeStatus) {
  ...
  if (status === "rejected") return { label: "Rejected", className: "bg-card text-charcoal/75" };
  return { label: "Proposed", className: "bg-sage/35 text-forest" };
}
  ...
  return (
    <section className="space-y-5 sm:space-y-6">
      ...
      <div className="flex gap-2 rounded-3xl border border-border/70 bg-card/85 p-2 shadow-soft">
        <button
          ...
          className={cn(
            ...
            tab === "open" ? "bg-sage/45 text-forest" : "text-charcoal/75 hover:bg-offwhite",
          )}
        >
          Open Trades
        ...
        <button
          ...
          className={cn(
            "flex-1 rounded-2xl px-4 py-2 text-sm font-medium transition",
            tab === "mine" ? "bg-gold/20 text-forest" : "text-charcoal/75 hover:bg-offwhite",
          )}
        >
          My Proposals
                  ...
                  {data.isCommissioner &&
                    (trade.status === "proposed" || trade.status === "commissioner_pending") && (
                      <div className="rounded-2xl border border-border/70 bg-card p-3">
                        <label className="text-xs text-charcoal/65">Commissioner comment</label>
                        <textarea
      ...
      <button
        ...
        className={cn(
          "fixed bottom-28 right-6 z-40 inline-flex h-14 items-center gap-2 rounded-3xl bg-sage px-5 text-sm font-semibold text-forest shadow-glow",
          "transition hover:-translate-y-0.5 hover:bg-sage/85",
        )}
      >
          ...
          <div className="space-y-4 px-6 pb-6">
            ...
            {!stepLoading && step === 1 && (
              <div className="space-y-3">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        selectedOfferPlayers.includes(player.id)
                          ...
                          : "border-border bg-offwhite text-charcoal",
                      )}
                    >
                      {player.name}
                    </button>
                ...
                <label className="text-xs text-charcoal/65">Optional football item</label>
                <input
                  value={offeringItem}
            ...
            {!stepLoading && step === 2 && (
              <div className="space-y-3">
                ...
                <label className="text-xs text-charcoal/65">Optional football item request</label>
                <input
